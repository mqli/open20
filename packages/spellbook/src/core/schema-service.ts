...
export interface RawSpell extends Partial<
  Omit<Spell, 'components' | 'classes' | 'level' | 'description'>
> {
  id?: string;
  name: string;
  level?: number | string;
...
export class SchemaService {
  ...
  private static normalizeClasses(classes: string[]): string[] {
    return classes
      .map((c) => {
        const found = this.ALL_SPELLCASTING_CLASSES.find(
          (ac) => ac.toLowerCase() === c.trim().toLowerCase(),
        );
        return found;
      })
  ...
  private static inferClassesFromDescription(description: string | string[]): string[] {
    ...
    const headerMatch = desc.match(
      /^(?:level\s+\d+\s+|[\d\w-]+-level\s+)?[a-z]+\s+(?:cantrip\s+)?\(([^)]+)\)/i,
    );
    if (headerMatch?.[1]) {
