import { useMemo } from 'react';
import { calculateSpellSlots, type Spell, type SpellLevel } from 'open20-core';
import {
  getCasterTypeForClass,
  getMatchingClassIds,
  getSpellClassStates,
  getAvailableSlots,
  canCastSpellWithSlots,
  getBestSpellAttackBonus,
  pickBestClassId,
  knowsSpell,
  isSpellPrepared,
} from 'open20-core/spells';
import type { Character, RecomputeDerivedStatsDeps } from 'open20-core';

/** Check if a spell is known through a feat (e.g. Magic Initiate). */
function isKnownViaFeat(char: Character, spellId: string): boolean {
  if (!char?.spells?.featSpells) return false;
  for (const entry of Object.values(char.spells.featSpells)) {
    if (entry.cantrips.includes(spellId) || entry.preparedSpells.includes(spellId)) return true;
  }
  return false;
}

export interface SpellCapabilities {
  // Basic status
  isKnown: boolean;
  isPrepared: boolean;
  isCantripKnown: boolean;
  isClassSpell: boolean;
  isConcentratingOnThis: boolean;

  // Casting capability (spellbook-caster-aware)
  knows: boolean;
  canCast: boolean;

  // Which buttons to show
  showPrepareButton: boolean;
  showLearnButton: boolean;
  showCantripButton: boolean;

  // Slot info
  hasRegularSlot: boolean;
  hasPactSlot: boolean;
  isWarlock: boolean;

  // Class matching (for multi-class dropdowns)
  matchingClassIds: string[];
  /** Subset of matchingClassIds whose individual class level can access this spell level */
  accessibleClassIds: string[];
  preparedClassIds: string[];
  alwaysPreparedClassIds: string[];
  cantripKnownClassIds: string[];

  // Derived stats (for single-class; multiclass uses per-class in UI components)
  spellAttackBonus: number;
}

// Module-level constant to avoid re-creating on every memo invalidation
const EMPTY_CAPABILITIES: SpellCapabilities = {
  isKnown: false,
  isPrepared: false,
  isCantripKnown: false,
  isClassSpell: false,
  isConcentratingOnThis: false,
  knows: false,
  canCast: false,
  showPrepareButton: false,
  showLearnButton: false,
  showCantripButton: false,
  hasRegularSlot: false,
  hasPactSlot: false,
  isWarlock: false,
  matchingClassIds: [],
  accessibleClassIds: [],
  preparedClassIds: [],
  alwaysPreparedClassIds: [],
  cantripKnownClassIds: [],
  spellAttackBonus: 0,
};

/**
 * Compute spell capabilities for a character.
 *
 * @param spell - The spell to evaluate.
 * @param character - The character to evaluate against (null-safe).
 * @param deps - Derived stats dependencies. **Must be referentially stable** — the caller
 *   should memoize this value (e.g. with `useMemo`) to avoid invalidating the internal
 *   `useMemo` on every render.
 */
export function useSpellCapabilities(
  spell: Spell | null | undefined,
  character: Character | null | undefined,
  deps: RecomputeDerivedStatsDeps,
): SpellCapabilities {
  return useMemo(() => {
    if (!character || !spell) return EMPTY_CAPABILITIES;

    // ── matching class IDs ──
    const matchingClassIds = getMatchingClassIds(character, spell);

    // ── spell class states (replaces 3 separate filters) ──
    const classStates = getSpellClassStates(character, spell.id);
    const preparedClassIds = classStates
      .filter((s) => s.isPrepared || s.isAlwaysPrepared)
      .map((s) => s.classId);
    const alwaysPreparedClassIds = classStates
      .filter((s) => s.isAlwaysPrepared)
      .map((s) => s.classId);
    const cantripKnownClassIds = classStates.filter((s) => s.isCantripKnown).map((s) => s.classId);

    // ── basic status ──
    const isKnown = knowsSpell(character, spell.id);
    const isFeatSpell = isKnownViaFeat(character, spell.id);
    const isPrepared = isSpellPrepared(character, spell.id) || isFeatSpell;
    // Feat spells are always prepared — treat them as always-prepared for badges / UI
    const resolvedAlwaysPreparedIds = isFeatSpell
      ? [...alwaysPreparedClassIds, '@feat']
      : alwaysPreparedClassIds;
    const isCantripKnown = cantripKnownClassIds.length > 0;
    const isClassSpell = matchingClassIds.length > 0;
    const isConcentratingOnThis = character.concentration?.spellId === spell.id;

    // ── knows (spellbook-caster-aware) ──
    // For spellbook casters (Wizard), must have learned the spell.
    // For other casters (Cleric, Druid, etc.), all class spells are "known".
    // In multiclass: if at least one non-spellbook caster matches, the spell is known.
    const knows =
      matchingClassIds.some((classId) => {
        const ct = getCasterTypeForClass(classId, deps);
        return !ct.canLearn;
      }) || isKnown;

    // ── slots ──
    const slotInfo = getAvailableSlots(character, spell.level as SpellLevel);
    const hasRegularSlot = slotInfo.hasRegularSlot;
    const hasPactSlot = slotInfo.hasPactSlot;
    const isWarlock = (character.classes ?? []).some((c) => c.classId === 'Warlock');

    // ── canCast (enhanced: checks slots) ──
    const canCast = canCastSpellWithSlots(character, spell);

    // ── spell attack bonus ──
    const statsClassId = pickBestClassId(character, matchingClassIds);
    const spellAttackBonus = statsClassId
      ? (character.spells.classSpellcasting[statsClassId]?.spellAttackBonus ?? 0)
      : getBestSpellAttackBonus(character);

    // ── button visibility ──
    // Character must be high enough level to access this spell level.
    // For multiclass: each class can only prepare/learn spells up to the level
    // that the individual class level supports, regardless of combined spell slots.
    const canAccessSpellLevel =
      spell.level === 0 ||
      matchingClassIds.some((classId) => {
        const charClass = character.classes.find((c) => c.classId === classId);
        if (!charClass) return false;
        const classSlots = calculateSpellSlots(classId, charClass.level, deps.classes ?? {});
        return (classSlots[spell.level]?.total ?? 0) > 0;
      });

    // Filter matchingClassIds to only those whose individual class level can access this spell level.
    // E.g., Cleric 5 / Wizard 1: a 3rd-level spell on both lists would have accessibleClassIds = ['Cleric']
    const accessibleClassIds =
      spell.level === 0
        ? matchingClassIds
        : matchingClassIds.filter((classId) => {
            const charClass = character.classes.find((c) => c.classId === classId);
            if (!charClass) return false;
            const classSlots = calculateSpellSlots(classId, charClass.level, deps.classes ?? {});
            return (classSlots[spell.level]?.total ?? 0) > 0;
          });

    // Per-class canPrepare: at least one matching class allows changing spells on long rest/level up
    const canPrepare = matchingClassIds.some(
      (classId) => getCasterTypeForClass(classId, deps).canPrepare,
    );

    // Prepare button: show for prepared casters (canPrepare) when the spell is known/accessible.
    // For spellbook casters, must also have learned the spell first.
    const showPrepareButton =
      isClassSpell && canPrepare && spell.level > 0 && knows && canAccessSpellLevel;
    // Learn toggle: only for spells with at least one spellbook-caster matching class
    // that can actually access this spell level
    const showLearnButton =
      isClassSpell &&
      spell.level > 0 &&
      matchingClassIds.some((classId) => getCasterTypeForClass(classId, deps).canLearn) &&
      canAccessSpellLevel;
    const showCantripButton = isClassSpell && spell.level === 0;

    return {
      isKnown,
      isPrepared,
      isCantripKnown,
      isClassSpell,
      isConcentratingOnThis,
      knows,
      canCast,
      showPrepareButton,
      showLearnButton,
      showCantripButton,
      hasRegularSlot,
      hasPactSlot,
      isWarlock,
      matchingClassIds,
      accessibleClassIds,
      preparedClassIds,
      alwaysPreparedClassIds: resolvedAlwaysPreparedIds,
      cantripKnownClassIds,
      spellAttackBonus,
    };
  }, [character, spell, deps]);
}
