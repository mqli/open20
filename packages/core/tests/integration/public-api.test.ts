...
describe('public API facade', () => {
  test('root facade includes all runtime exports from public modules', () => {
    const moduleExports = [character, data, engine, monster, rolls, spells, storage];
  ...
  test('getSpell is exported as canonical root spell query API', () => {
    expect(root).toHaveProperty('getSpell');
    expect(root.getSpell).toBe(spells.getSpell);
  });
