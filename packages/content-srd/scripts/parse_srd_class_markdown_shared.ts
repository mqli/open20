...
function parseLevelFeatures(lines: string[]): Map<number, { name: string; description: string }[]> {
  ...
  while (i < lines.length) {
    ...
    const name = stripMarkdownHeading(lines[i])
      .replace(/^Level\s*\d+:\s*/i, '')
      .trim();
    let j = i + 1;
    while (
      j < lines.length &&
      !/^#{3,4}\s*Level\s*\d+:/i.test(lines[j]) &&
      !/^###\s+.+Subclass:/i.test(lines[j])
    ) {
      j += 1;
    }
...
function parseSubclasses(
  lines: string[],
  classId: string,
  spellByNameKey: Map<string, string>,
): Subclass[] {
  ...
  const grantedAtLevel =
    classId === 'Cleric' || classId === 'Warlock' ? 1 : classId === 'Wizard' ? 2 : 3;
...
export function parseClassMarkdownContent(
  content: string,
  spellByNameKey: Map<string, string>,
): ParsedClassDocument {
  ...
  const levelFeatures = parseLevelFeatures(
    lines.slice(
      0,
      lines.findIndex((line) => /^###\s+.+Subclass:/i.test(line)) > -1
        ? lines.findIndex((line) => /^###\s+.+Subclass:/i.test(line))
        : lines.length,
    ),
  );
...
export function buildSpellNameKeyMap(
